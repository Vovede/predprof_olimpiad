...
class Monitoringiop(QMainWindow):
    def __init__(self):
        ...
        self.sef = None
        self.flag = True
        self.list_weather = ['Погода хорошая', 'Погода плохая', 'Погода норм']
        self.navTab = {
        ...
        self.clearBtnTableWidget.clicked.connect(self.clear_adres)
        self.pushButton_7.clicked.connect(self.url_append)
        self.pushButton_11.clicked.connect(self.conclusion)
        #self.tableWidget.itemChanged.connect(self.item_changed)

    def conclusion(self):
        self.textEdit_2.setPlainText(self.list_weather[0])



    def url_append(self):
        try:
            # C:\Users\Roma\PycharmProjects\pythonProject37\films_db(1).sqlite
            if self.lineEdit.text() != '' and self.lineEdit.text() != 'Адрес':
                if '.sqlite' in self.lineEdit.text():
                    n1 = str(self.lineEdit.text()).replace('/', '/')
                    with open(n1, 'r') as f:
                        print('Работает')
                        # Зададим тип базы данных
                    db = QSqlDatabase.addDatabase('QSQLITE')
                        # Укажем имя базы данных
                    db.setDatabaseName(n1)
                    ...
                    self.lineEdit.setText('Файл не найден')
            else:
                self.lineEdit.setText('Файл пуст или ссылка записана неправильно')
        except BaseException:
            self.lineEdit.setText('Неправильный доступ к файлу')

    def clear_adres(self):
        if self.lineEdit.text() == 'Адрес' or self.lineEdit.text() != '':
